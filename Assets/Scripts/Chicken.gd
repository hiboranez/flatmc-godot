...
# 预加载
@onready var entity_model_mesh =$SubViewportContainer/SubViewport/Chicken/Bones_Chicken/Skeleton3D/Mob_Chicken
@onready var animation_tree = $SubViewportContainer/SubViewport/AnimationTree
@onready var entity_sprite = $Sprite2D
@onready var entity_model = $SubViewportContainer/SubViewport/Chicken
...
func init(args):
	...
	update_target_pos()
	StaticLoad.game.sound_audio_manager.play_random_audio_at_position("chicken", "say", position, 1)
...
func update_animation_tree():
	...
	animation_tree["parameters/Swing/blend_amount"] = animation_tree_parameters["swing"]
...
func update_animation_by_data():
	...
	if not is_on_floor() and not (is_on_ladder or is_frozen or is_flying):
		animation_tree_parameters["swing"] = lerpf(animation_tree_parameters["swing"], 1, StaticLoad.BLEND_SPEED*delta)
	else:
		animation_tree_parameters["swing"] = lerpf(animation_tree_parameters["swing"], 0, StaticLoad.BLEND_SPEED*delta)
	
	var detect_size = 40
	if move_state == "run":
		detect_size = 100
	var half_detect_size = 17+(detect_size/2)
	...
	if turn_state > 0:
		if up_area_collision_shape.position.x < 0:
			up_area_collision_shape.position.x = 39
...
func update_local_gravity():
	...
	if not is_on_floor():
		if velocity.y > 200:
			var diff = velocity.y - 200
			var max_diff = 200*get_process_delta_time()*20
			if diff >= max_diff:
				diff = max_diff
			velocity += Vector2(0, -diff)
			if velocity.y < 200:
...
func get_damage(args):
	...
	if health <= 0:
		...
		StaticLoad.game.sound_audio_manager.play_random_audio_at_position("chicken", "hurt", position, 1)
...
func die(reason, object):
	...
	if not StaticLoad.is_muti_mode or (StaticLoad.is_muti_mode and multiplayer.get_unique_id() == 1):
		var droppped_item_list = StaticLoad.get_dropped_item_by_name("entity", "CHICKEN", "others")
...
func set_entity_model_skin_by_texture(got_skin_texture):
	var entity_material = load("res://Assets/Materials/Chicken.tres").duplicate(true)
