...
func connect_server():
	if StaticLoad.is_lan_server:
		...
		var server_info = server_config.load_encrypted_pass(StaticLoad.server_path+"/"+StaticLoad.select_server+".srv", SettingsManager.get_default_value("config_password"))
		if server_info != OK:
			return
...
func load_scene():
	...
	if scene_load_status == ResourceLoader.THREAD_LOAD_LOADED:
		...
		set_process(false)
		SceneManager.change_scene(ResourceLoader.load_threaded_get(game_path))
