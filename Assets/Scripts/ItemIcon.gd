...
func init_icon(block_name):
	...
	if StaticLoad.get_item_model_type_by_name(block_name.to_upper()) == 1 or StaticLoad.get_item_model_type_by_name(block_name.to_upper()) == 2:
		if block_name.to_upper().contains("SPAWN_EGG"):
			var item_texture = load("res://assets/ResourcePacks/"+SettingsManager.get_current_setting("resource_pack")+"/Items/spawn_egg.png") as Texture2D
			var item_texture_top = load("res://assets/ResourcePacks/"+SettingsManager.get_current_setting("resource_pack")+"/Items/spawn_egg_overlay.png") as Texture2D
			...
			var item_texture = load("res://assets/ResourcePacks/"+SettingsManager.get_current_setting("resource_pack")+"/Items/"+block_name.to_lower()+".png") as Texture2D
