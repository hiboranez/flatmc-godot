...
func create_world(world_name: String):
	...
	var world_seed = ""
	if setting_vboxcontainer.has_node("Seed"):
		world_seed = setting_vboxcontainer.get_node("Seed").get_line_edit_text()
	...
	if setting_vboxcontainer.has_node("WorldType"):
		world_type = setting_vboxcontainer.get_node("WorldType").get_option_button_text()
	if setting_vboxcontainer.has_node("Gamemode"):
		gamemode = setting_vboxcontainer.get_node("Gamemode").get_option_button_text()
	if setting_vboxcontainer.has_node("AllowCheat"):
		allow_cheat = setting_vboxcontainer.get_node("AllowCheat").get_option_button_text()
	if setting_vboxcontainer.has_node("Achievement"):
		achievement = setting_vboxcontainer.get_node("Achievement").get_option_button_text()
	...
	for x in range(-1,1):
		for y in range(-1,1):
			...
			var chunk = get_node("/root/WorldManager").generate_chunk(Vector2i(x, y), world_seed, world_type)
			var value_dict = {
				"blocks" : chunk[0],
...
func _on_create_world_menu_create_world_button_pressed() -> void:
	...
	if setting_vboxcontainer.has_node("WorldName"):
		world_name = setting_vboxcontainer.get_node("WorldName").get_line_edit_text() 
