...
func _ready() -> void:
	...
	ActionManager.register_action("block_selection_frame", "set_frame_restricted_position", set_frame_restricted_position)
...
func set_frame_restricted_position(frame_position: Vector2) -> void:
	if ClientManager.local_player == null:
		return
	if ClientManager.local_player.gamemode == "creative":
		set_frame_position(frame_position)
	else:
		var restricted_block_selection_position = ClientManager.get_restricted_block_selection_position(frame_position)
		set_frame_position(restricted_block_selection_position)

func set_frame_position(frame_position: Vector2) -> void:
	var x_offset = 25
	var y_offset = -25
	...
	position = Vector2((int(frame_position.x)/50)*50+x_offset, (int(frame_position.y)/50)*50+y_offset)
	if ClientManager.local_player != null:
		...
		if frame_position.y > 0:
			block_y_offset = 0
		@warning_ignore("integer_division")
		var block_pos = Vector2i(int(frame_position.x)/50+block_x_offset, int(frame_position.y)/50+block_y_offset)
		ClientManager.local_player.selected_block_pos = block_pos

func set_frame_coordinate(frame_coordinate: Vector2i) -> void:
	var x_offset = 25
	var y_offset = -25
	if frame_coordinate.x < 0:
		x_offset = -25
	if frame_coordinate.y > 0:
		y_offset = 25
	position = Vector2((frame_coordinate.x)*50+x_offset, (frame_coordinate.y/50)*50+y_offset)
	if ClientManager.local_player != null:
		ClientManager.local_player.selected_block_pos = frame_coordinate
