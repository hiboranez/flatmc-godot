...
func load_settings():
	...
	if result != OK:
		return
	for setting_node in settings_vboxcontainer.get_children():
		if setting_node.has_method("load_setting"):
			setting_node.load_setting(config)

func _on_settings_menu_save_button_pressed() -> void:
	AudioManager.play_static_audio("sound/ui/click")
	if settings_vboxcontainer.has_node("PlayerName"):
		var player_name = settings_vboxcontainer.get_node("PlayerName").get_line_edit_text()
		if player_name == "":
