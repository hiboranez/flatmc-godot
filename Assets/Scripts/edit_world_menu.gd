...
func _ready() -> void:
	if not has_node("/root/SingleMenu"):
		return
	var selected_world_name = get_node("/root/SingleMenu").selected_world
	if settings_vboxcontainer.has_node("WorldName"):
		settings_vboxcontainer.get_node("WorldName").set_line_edit_text(selected_world_name)
	...
	if settings_vboxcontainer.has_node("AllowCheat"):
		settings_vboxcontainer.get_node("AllowCheat").set_option_button_text(read_allow_cheat)
	if settings_vboxcontainer.has_node("Achievement"):
		settings_vboxcontainer.get_node("Achievement").set_option_button_text(read_achievement)

func _on_edit_world_menu_confirm_button_pressed() -> void:
	AudioManager.play_static_audio("sound/ui/click")
	if not has_node("/root/SingleMenu"):
		return
	var old_world_name = get_node("/root/SingleMenu").selected_world
	var new_world_name = ""
	if settings_vboxcontainer.has_node("WorldName"):
		new_world_name = settings_vboxcontainer.get_node("WorldName").get_line_edit_text()
	var world_list_path = SettingsManager.get_default_value("world_list_path")
	...
	if new_world_name == "":
		...
		return
	if new_world_name != old_world_name and DirAccess.dir_exists_absolute(new_world_name):
	...
	if settings_vboxcontainer.has_node("AllowCheat"):
		read_allow_cheat = settings_vboxcontainer.get_node("AllowCheat").get_option_button_text()
	if settings_vboxcontainer.has_node("Achievement"):
		read_achievement = settings_vboxcontainer.get_node("Achievement").get_option_button_text()
