...
# 预加载
@onready var entity_model_mesh = $SubViewportContainer/SubViewport/Sheep/Bones_Sheep/Skeleton3D/Mob_Sheep
@onready var animation_tree = $SubViewportContainer/SubViewport/AnimationTree
@onready var entity_sprite = $Sprite2D
@onready var entity_model = $SubViewportContainer/SubViewport/Sheep
...
func init(args):
	...
	update_target_pos()
	StaticLoad.game.sound_audio_manager.play_random_audio_at_position("sheep", "say", position, 1)
...
func die(reason, object):
	...
	if not StaticLoad.is_muti_mode or (StaticLoad.is_muti_mode and multiplayer.get_unique_id() == 1):
		var droppped_item_list = StaticLoad.get_dropped_item_by_name("entity", "SHEEP", "others")
