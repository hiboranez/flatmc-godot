...
func add_server(server_name: String):
	...
	server.save_encrypted_pass(server_path+server_name+".srv", SettingsManager.get_default_value("config_password"))
	StaticLoad.select_server = null
...
func edit_server(server_name: String):
	...
	var server_config = ConfigFile.new()
	var server_info = server_config.load_encrypted_pass(delete_path, SettingsManager.get_default_value("config_password"))
	if server_info != OK:
		return
	var icon_buffer = server_config.get_value("server", "icon")
	var server = ConfigFile.new()
	...
	server.save_encrypted_pass(save_path, SettingsManager.get_default_value("config_password"))
	StaticLoad.select_server = null
...
func add_lan_server(data):
	var lan_server = SceneManager.get_scene("lan_server").instantiate()
...
func _on_muti_menu_join_server_button_pressed() -> void:
	...
	for server_detect in StaticLoad.server_detects.get_children():
		server_detect.disconnect_and_free()
	SceneManager.change_scene("loading_server_menu")
...
func _on_muti_menu_delete_server_button_pressed() -> void:
	...
	if StaticLoad.select_server == null:
		return
	SceneManager.pop_secondary_confirmation(self, StaticLoad.select_server + tr("SECONDARY_CONFIRMATION_1"), Callable(self, "delete_server").bind(StaticLoad.select_server))
