...
# 预加载
@onready var entity_model_mesh = $SubViewportContainer/SubViewport/Zombie/Zombie_Bones/Skeleton3D/Zombie
@onready var animation_tree = $SubViewportContainer/SubViewport/AnimationTree
@onready var entity_sprite = $Sprite2D
@onready var entity_model = $SubViewportContainer/SubViewport/Zombie
...
func init(args):
	...
	update_target_pos()
	StaticLoad.game.sound_audio_manager.play_random_audio_at_position("zombie", "say", position, 1)
...
func get_damage(args):
	...
	if health <= 0:
		...
		StaticLoad.game.sound_audio_manager.play_random_audio_at_position("zombie", "hurt", position, 1)
...
func die(reason, object):
	...
	StaticLoad.game.sound_audio_manager.play_random_audio_at_position("zombie", "death", position, 1)
	...
	if not StaticLoad.is_muti_mode or (StaticLoad.is_muti_mode and multiplayer.get_unique_id() == 1):
		var droppped_item_list = StaticLoad.get_dropped_item_by_name("entity", "ZOMBIE", "others")
