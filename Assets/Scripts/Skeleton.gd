...
# 预加载
@onready var entity_model_mesh = $"SubViewportContainer/SubViewport/Skeleton/Skeleton-Bones/Skeleton3D/Skeleton-Mesh"
@onready var animation_tree = $SubViewportContainer/SubViewport/AnimationTree
@onready var entity_sprite = $Sprite2D
@onready var entity_model = $SubViewportContainer/SubViewport/Skeleton
...
@onready var item_in_hand = $"SubViewportContainer/SubViewport/Skeleton/Skeleton-Bones/Skeleton3D/BoneAttachment3D/ItemInHand"

# 实体变量
var uuid = UUID.v4()
var entity_type = "skeleton"
...
func init(args):
	...
	update_target_pos()
	AudioManager.play_random_audio_at_position("sound/skeleton/say", position, 1)
...
func update_animation_by_data():
	...
	if is_attacking:
		#if not animation_tree["parameters/Attack/active"]:
	...
	if turn_state > 0:
		...
		if ground_area1_collision_shape.position.x < 0:
			ground_area1_collision_shape.position.x = 14
		if ground_area2_collision_shape.position.x < 0:
			ground_area2_collision_shape.position.x = 59
	else:
		if up_area_collision_shape.position.x > 0:
			up_area_collision_shape.position.x = -23
		if down_area_collision_shape.position.x > 0:
...
func set_item_in_hand(got_item_name):
	item_in_hand.set_item_in_hand(got_item_name, false)
...
func update_local_fire_damage_by_data():
	...
	if fire_lasting_timer > 0 and not is_on_fire:
		is_on_fire = true
		fire_damage_timer = fire_damage_time/2
...
func attack():
	...
	if target_entity.position.x < position.x:
		side = "right"
	if target_entity.get_entity_type() == "player":
		if target_entity.gamemode == "creative":
			return
...
func update_local_refresh_target_timer():
	...
	if panic_timer <= 0:
		...
		return
	if refresh_target_entity_timer <= 0:
		var player_can_trace_list = []
		for player_tmp in StaticLoad.game.players.get_children():
			if player_tmp.get_is_dead():
				if target_entity == player_tmp:
					target_entity = null
				continue
...
func update_state_dict():
	...
	state_dict["is_attacking"] = is_attacking
...
func get_damage(args):
	...
	if health <= 0:
		...
		AudioManager.play_random_audio_at_position("sound/skeleton/hurt", position, 1)
...
func die(reason, object):
	...
	AudioManager.play_random_audio_at_position("sound/skeleton/death", position, 1)
